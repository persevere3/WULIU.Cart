<template>
  ...
  <div class="confirm" v-if="isConfirmToPay">
    <div class="frame">
      ...
      <div class="message">
        <template
          v-if="
            pay_method != 'PayOnDelivery' && pay_method != 'MartPayOnDelivery'
          "
        >
          前往付款頁面
        </template>
      ...
      <div class="buttonGroup">
        <div
          ...
          @click=";(isConfirmToPay = false), toPay()"
        >
          確定
        </div>
      </div>
  ...
  <div class="confirm" v-if="isConfirmDiscountCodeUsed">
    <div class="frame">
      ...
      <div class="message">
        您已使用過此折扣碼！請按取消後重新輸入
        <span v-if="!user_account"> 手機或 </span>
        折扣碼，或按確定放棄優惠直接完成訂單
      </div>
      <div class="buttonGroup">
        <div class="button cancel" @click="isConfirmDiscountCodeUsed = false">
          取消
        </div>
        <div class="button determine" @click="cancelDiscountCodeCreateOrder()">
          確定
        </div>
      </div>
    </div>
    ...
    <div class="frame">
      ...
      <div class="message bank">
        ...
        <div class="bank_account">
          ...
          />
          <div class="copy" @click="useCopy(store.SelfAtmId, '.copy_input')">
  ...
  <div class="confirm" v-if="isConfirmIsRegister">
    <div class="frame">
      ...
      <div class="buttonGroup">
        <div
          class="button cancel"
          @click=";(isConfirmIsRegister = false), toPay()"
        >
          否
          <template
            v-if="
        ...
        <div
          class="button determine"
          @click=";(isConfirmIsRegister = false), (isConfirmRegister = true)"
        >
          是，請填寫註冊資料
        </div>
      </div>
...
let { getPathname } = useCartCommon()
let { ECPay_form_value, ECPay_store_form_value } = storeToRefs(useCart())
